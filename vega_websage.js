...
WebSAGE.SetIniExtended = function (inksage_labelvec, lbv, item) {
  var j
  ...
  function parsev3(spec) {
    ...
    item.vw.runAfter(function () {
      ...
      pms.then(function (svgstr) {
        ...
        item.vg._groups[0][0].setAttributeNS(
          null,
          'transform',
        ...
        item.inittransform = item.vg._groups[0][0].getAttributeNS(null, 'transform')
  ...
  switch (inksage_labelvec[lbv].tag) {
    case '#arc': // arc for donut like graphics
      WebSAGE.addPointToList(inksage_labelvec[lbv].src)
    ...
    case '#vega-lite': // vega-lite V1 or V2 chart, defined under a rectangle
      {
        ...
        for (j = 0; j < item.pnts.length; j++) {
          WebSAGE.addPointToList(item.pnts[j])
        }

        function procvegalite(data) {
          var spc
          if (typeof data === 'object') {
            spc = vegaLite.compile(data).spec
            spc['width'] = spc['width'] || 100
            spc['height'] = spc['height'] || 100
          } else {
            var obj = JSON.parse(data)
            spc = vegaLite.compile(obj).spec
        ...
        if (inksage_labelvec[lbv].prompt.indexOf('{') == 0)
          ...
          $.get(
            inksage_labelvec[lbv].prompt + '?' + new Date().getTime(),
            procvegalite
          )
      }
      break
    ...
    case '#vega5': // vega V5 charts, defined under a rectangle
      {
        item.vgTableName = []
        item.style.display = 'none' // hide the rectangle
        // point list in the Source field
        ...
        if (srcsplit.length > 1) {
          ...
          if (paramsplit.length == 0) item.timespan = item.width.baseVal.value
          else item.timespan = paramsplit[0]
        } else item.timespan = item.width.baseVal.value
        ...
        if (inksage_labelvec[lbv].prompt.indexOf('{') == 0) {
          ...
          $.get(
            ...
            function (data) {
              var spc
              if (typeof data === 'object') spc = data
              else spc = JSON.parse(data)
              spc['width'] = spc['width'] || 100
              spc['height'] = spc['height'] || 100
              parsev3(spc)
            }
    ...
    case '#vega-json': // vega charts, updated by JSON file obtained periodically, defined under a rectangle
    ...
    case '#vega5-json': // vega V5 charts, updated by JSON file obtained periodically, defined under a rectangle
      {
        item.vgTableName = []
        item.style.display = 'none' // hide the rectangle
            ...
            if (typeof spc.data[0].url !== 'undefined') {
              if (typeof spc.data[0].update_period != 'undefined')
                setInterval(function () {
                  $.get(spc.data[0].url, function (data) {
          ...
          if (spc.data.length > 1)
            if (typeof spc.data[1].url !== 'undefined') {
              if (typeof spc.data[1].update_period != 'undefined')
                setInterval(function () {
                  $.get(spc.data[1].url, function (data) {
                    if (typeof data === 'string')
                      item.vw
                        .change(
                          spc.data[1].name,
...
WebSAGE.SetExeExtended = function (i) {
  var j
  var newdata
  switch (WebSAGE.InkSage[i].tag) {
    case '#arc':
      var vt = WebSAGE.valueResolveCoded(
        WebSAGE.InkSage[i].parent._d3arc_tag,
    ...
    case '#vega': // vega V2 chart, defined under a rectangle
      if (
        typeof WebSAGE.InkSage[i].parent.vw != 'undefined' &&
        typeof WebSAGE.InkSage[i].parent.vgInitData != 'undefined' &&
        WebSAGE.InkSage[i].parent.vgInitData != ''
      ) {
        WebSAGE.InkSage[i].parent.vw
          .data(WebSAGE.InkSage[i].parent.vgTableName[0])
          .remove(function (d) {
            return true
          })
        ...
        newdata = JSON.parse(
          JSON.stringify(WebSAGE.InkSage[i].parent.vgInitData)
        )
      }
            ...
            try {
              if (typeof vl === 'string') {
                if (vl.indexOf('HIS#') >= 0) {
                  ...
                  if (
                    WebSAGE.InkSage[i].hasOwnProperty('valores') &&
                    typeof WebSAGE.InkSage[i].valores[pnt] != 'undefined'
                  ) {
                    for (
                      j = 0;
                      j < WebSAGE.InkSage[i].valores[pnt].length;
                      j++
                    ) {
                      newdata.push({
                        pnt: '' + pnt,
                    ...
                    for (
                      var indv = WebSAGE.InkSage[i].valores[pnt].length - 1;
                      indv >= 0;
                      indv--
                    ) {
                      var secdif =
                        (d.getTime() - WebSAGE.InkSage[i].datas[pnt][indv]) /
                        1000
                    ...
                    var utm =
                      new Date().getTime() -
                      WebSAGE.InkSage[i].parent.timespan * 60 * 1000
                    if (
                      !WebSAGE.InkSage[i].parent.hasOwnProperty('histCalls')
                    ) {
                      WebSAGE.InkSage[i].parent.histCalls = []
                    }
                    if (
                      !WebSAGE.InkSage[i].parent.histCalls.hasOwnProperty(pnt)
                    ) {
                      var calltps =
                        '$.getScript( WebSAGE.g_timePntServer + "?P=' +
                        pnt +
                        '&U=' +
                        utm / 1000 +
                        '&F=S' +
                ...
                if (vl.indexOf('PNT#') >= 0) {
                  if (
                    WebSAGE.InkSage[i].parent.pnts.length >
                    vl.split('#')[1] - 1
                  ) {
                    // testa se existem mais pontos no arquivo json que ponto linkados em SAGE/source
                    var vt = WebSAGE.valueResolveCoded(
                      WebSAGE.InkSage[i].parent.pnts[vl.split('#')[1] - 1],
                      WebSAGE.InkSage[i].parent
                    )
                    if (vt !== '????') newdata[index][ix] = vt
                    else newdata.splice(-1, 1)
                  } else newdata.splice(-1, 1)
                  ...
                  if (
                    ...
                    // testa se existem mais pontos no arquivo json que ponto linkados em SAGE/source
                    newdata[index][ix] = WebSAGE.getFlags(
                      WebSAGE.InkSage[i].parent.pnts[vl.split('#')[1] - 1]
                    )
                  ...
                  if (
                    ...
                    // testa se existem mais pontos no arquivo json que ponto linkados em SAGE/source
                    newdata[index][ix] = WebSAGE.getInfLim(
                      WebSAGE.InkSage[i].parent.pnts[vl.split('#')[1] - 1]
                    )
                  ...
                  if (
                    ...
                    // testa se existem mais pontos no arquivo json que ponto linkados em SAGE/source
                    newdata[index][ix] = WebSAGE.getSupLim(
                      WebSAGE.InkSage[i].parent.pnts[vl.split('#')[1] - 1]
                    )
                  ...
                  if (
                    ...
                    // testa se existem mais pontos no arquivo json que ponto linkados em SAGE/source
                    newdata[index][ix] = WebSAGE.getDescription(
                      WebSAGE.InkSage[i].parent.pnts[vl.split('#')[1] - 1]
                    )
                  ...
                  if (
                    ...
                    // testa se existem mais pontos no arquivo json que ponto linkados em SAGE/source
                    newdata[index][ix] = WebSAGE.getBay(
                      WebSAGE.InkSage[i].parent.pnts[vl.split('#')[1] - 1]
                    )
                  ...
                  if (
                    ...
                    // testa se existem mais pontos no arquivo json que ponto linkados em SAGE/source
                    newdata[index][ix] = WebSAGE.getSubstation(
                      WebSAGE.InkSage[i].parent.pnts[vl.split('#')[1] - 1]
                    )
                }
              }
      ...
      if (typeof WebSAGE.InkSage[i].parent.vw != 'undefined') {
        ...
        WebSAGE.InkSage[i].parent.vg.html(
          $('#' + WebSAGE.InkSage[i].parent.vgId + ' .vega')[0].childNodes[0]
    ...
    case '#vega4': // vega V4 chart, defined under a rectangle
                  ...
                  if (
                    ...
                    for (
                      ...
                      if (WebSAGE.InkSage[i].parent.timespan > 0) {
                        ...
                        if (
                          WebSAGE.InkSage[i].datas[pnt][indv] >
                          WebSAGE.InkSage[i].parent.datafim
                        ) {
                          // time now > previwed time end
                          WebSAGE.InkSage[i].valores.forEach(function (
                            element,
                            index,
                            arr
                          ) {
                          ...
                          WebSAGE.InkSage[i].datas.forEach(function (
                            element,
                            index,
                            arr
                          ) {
                          ...
                          WebSAGE.InkSage[i].parent.dataini =
                            new Date().getTime() -
                            (new Date().getTime() %
                              (Math.abs(
                                WebSAGE.InkSage[i].parent.timespan * 60
                              ) *
                                1000)) +
                    ...
                    if (WebSAGE.InkSage[i].parent.timespan < 0) {
                      WebSAGE.InkSage[i].parent.dataini =
                        new Date().getTime() -
                        (new Date().getTime() %
                          (Math.abs(WebSAGE.InkSage[i].parent.timespan * 60) *
                            1000)) +
                        ((new Date().getTimezoneOffset() * 60 * 1000) %
                          (Math.abs(WebSAGE.InkSage[i].parent.timespan * 60) *
                            1000))
                      WebSAGE.InkSage[i].parent.datafim =
                        WebSAGE.InkSage[i].parent.dataini +
                        Math.abs(WebSAGE.InkSage[i].parent.timespan * 60 * 1000)
                    }
      ...
      if (typeof WebSAGE.InkSage[i].parent.vw != 'undefined') {
        WebSAGE.InkSage[i].parent.vw
          .change(
            WebSAGE.InkSage[i].parent.vgTableName[0],
            vega
              .changeset()
              .remove(function (d) {
                return true
